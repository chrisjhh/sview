...
export const vdot = function(distance, time) {
  ...
  for (; index<distances.length; ++index) {
    ...
    if (target > distance) {
      ...
      // Use an interpolated value if we can
      return interpolated_vdot(distance,time);
    }
  }
...
const interpolated_vdot = function(distance, time) {
  // Find distance entry
  let index = 0;
  for (; index<distances.length; ++index) {
    const target = distances[index];
  ...
  const interpolated_time = function(val,index) {
    const low = hms(table[val][index - 1]);
    const high = hms(table[val][index]);
    const d0 = distances[index - 1];
    const d1 = distances[index];
    return low + (high - low)*(distance-d0)/(d1-d0);
  };
  ...
  let above = 0;
  for (; val <=72; ++val) {
    below = interpolated_time(val,index);
    if (below < time) {
      break;
    }
    above = below;
