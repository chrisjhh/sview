  ...
  drawAxes() {
    ...
    if (this.ylabel != null) {
      ...
      switch(this.ylabel) {
        ...
        case 'altitude':
          let step = 10;
          if (this.ylabel === 'power') {
            step = 50;
          }
          for (let hr=step; hr<this.max_y; hr+=step) {
            if (hr < this.min_y) {
              continue;
            }
            ...
            ctx.moveTo(xn,yn);
            ctx.lineTo(xn-3,yn);
            // Don't display altitude label if they will be too close to max / min values
            if (this.ylabel === 'altitude' && yn - this.margins[0] < 10 || this.height - this.bottomMargin - yn < 10) {
              continue;
            }
            let xoffset = 20;
        ...
        case 'ridecadence':{
          let step =5;
          if (this.ylabel === 'ridecadence') {
            step = 10;
          }
          for (let cad=10; cad<this.max_y; cad+=step) {
            if (cad < this.min_y) {
              continue;
            }
            ...
            ctx.fillText(value.toString(),xn-xoffset,yn+3);
          }
          ctx.stroke();
          ctx.beginPath();
          ctx.setLineDash([2, 3]);
